...
[group("FrontEnd")]
front-tests-run: (run "tests_exe" default_sanitizers '-sf="*FrontEnd*"')
...
[group("MiddleEnd")]
middle-tests-run: (run "tests_exe" default_sanitizers '-sf="*MiddleEnd*"')
...
[group("ReverseEnd")]
reverse-tests-run: (run "tests_exe" default_sanitizers '-sf="*ReverseEnd*"')
...
[group("All")]
all-tests-run: (run "tests_exe" default_sanitizers)
...
build meson_target sanitizers: (ensure_build_dir_setup sanitizers)
    ...
    @echo "Compiled executable: $(just find_executable {{ sanitizers }} {{ meson_target }})"

[group("Meta")]
run meson_target sanitizers *ARGS: (build meson_target sanitizers)
    @just center_header "EXECUTING"
    @$(just find_executable {{ sanitizers }} {{ meson_target }}) {{ ARGS }}

[group("Utils")]
[private]
find_executable sanitizers meson_target:
