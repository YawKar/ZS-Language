...
static void WriteNodeDefinition(
    FILE* file, const LangNode_t* node, VariableArr* arr
);
static void WriteNodeConnection(
    FILE* file, const LangNode_t* from, const LangNode_t* to
);

static void WriteNumberNode(FILE* file, const LangNode_t* node);
static void WriteVariableNode(
    FILE* file, const LangNode_t* node, VariableArr* arr
);
...
void DoTreeInGraphviz(
    const LangNode_t* root, DumpInfo* info, VariableArr* arr
) {
    assert(root);
    assert(info);
    assert(arr);
...
static void WriteDotHeader(FILE* file) {
    ...
    fprintf(
        file,
        DOT_INDENT "node [shape=record, style=filled, fillcolor=lightblue];\n"
    );
...
static void WriteRootNode(FILE* file, const LangNode_t* root) {
    ...
    if (root->parent == NULL) {
        fprintf(
            file,
            DOT_INDENT "\"1\" [label=\"ROOT\", shape=rect, fillcolor=pink];\n"
        );
...
static void WriteNumberNode(FILE* file, const LangNode_t* node) {
    ...
    fprintf(
        file, "Left: %p | Right: %p\"", (void*)node->left, (void*)node->right
    );
...
static void WriteOperationNode(FILE* file, const LangNode_t* node) {
    ...
    fprintf(
        file,
        " | Value: %s | {Left: %p | Right: %p}}\" shape=Mrecord color=black "
...
static void WriteNodeConnection(
    FILE* file, const LangNode_t* from_node, const LangNode_t* to_node
) {
    assert(file);
    assert(from_node);
    assert(to_node);
