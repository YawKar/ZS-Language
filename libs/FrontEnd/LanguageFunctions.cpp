...
DifErrors ResizeArray(VariableArr* arr) {
    ...
    if (arr->size + 2 > arr->capacity) {
        ...
        VariableInfo* new_array =
            (VariableInfo*)calloc(arr->capacity, sizeof(VariableInfo));
        if (!new_array) {
            fprintf(stderr, "Memory error.\n");
            return kNoMemory;
        }
...
LangNode_t* NewVariable(
    ...
    if (!found_flag) {
        ...
        VariableArr->var_array[VariableArr->size].variable_name =
            strdup(variable);
...
DifErrors PrintAST(LangNode_t* node, FILE* file, VariableArr* arr, int indent) {
    ...
    switch (node->type) {
        ...
        case kVariable:
            fprintf(
                file, "\"%s\"", arr->var_array[node->value.pos].variable_name
            );
            break;
        case kOperation:
